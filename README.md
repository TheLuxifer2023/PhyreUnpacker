...
### Анализ системы шрифтов PhyreEngine

**📖 [PhyreEngine_Font_System_Analysis.md](PhyreEngine_Font_System_Analysis.md)** — подробный анализ системы шрифтов PhyreEngine

Полная документация работы PhyreEngine с шрифтами, включая:

- **Архитектура системы:** основные классы (`PBitmapFont`, `PBitmapFontCharInfo`, `PBitmapFontText`, `PBitmapTextMaterial`, `PBitmapTextMaterialSDF`)
- **Форматы файлов:** детальное описание структуры `.fgen` и `.phyre` файлов
- **Техники рендеринга:** 7 различных техник рендеринга текста (Alpha Blend, SDF Hard/Soft Edges, Outline, Shadow, Glow)
- **Шейдеры:** подробный разбор `PhyreText.fx` с оптимизациями для D3D11/GNM/GL
- **Инструменты разработки:** описание PhyreFontEditor и PhyreAssetProcessor
- **Процесс записи:** полный процесс создания `.phyre` файлов со шрифтами (5 этапов)
- **Структура `.phyre`:** детальный разбор заголовка кластера и секций данных
- **Шифрование и сжатие:** анализ защиты файлов (fixup компрессия метаданных)
- **Декомпрессия в SDK:** методы загрузки и декомпрессии `.phyre` файлов
